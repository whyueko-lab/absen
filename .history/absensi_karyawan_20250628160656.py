...
@st.cache_data
def load_data():
    return pd.read_excel("dataset_absensi_dengan_jam.xlsx")  # Pastikan file ini ada di repo yang sama
...
if len(np.unique(y_train)) < 2:
    st.error("❌ Data latih hanya punya satu kelas. SMOTE tidak bisa digunakan.")
    st.stop()

if np.isnan(X_train).sum() > 0 or np.isnan(y_train).sum() > 0:
    st.error("❌ Terdapat nilai kosong pada data. Tidak bisa lanjut.")
    st.stop()
...
# -----------------------------
    ...
    if submit:
        ...
        if hasil.lower() == "terlambat":
            st.error("⏰ **Prediksi: TERLAMBAT**\n\n📌 Karyawan kemungkinan akan datang *terlambat*.")
        else:
            st.success("✅ **Prediksi: TIDAK TERLAMBAT**\n\n👍 Karyawan diprediksi akan *tepat waktu*.")
